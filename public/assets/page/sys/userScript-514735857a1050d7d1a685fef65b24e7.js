$(function() {
	...
	var userTable = $("#example").DataTable({
		...
		"aoColumns" : [{
					...
					}, 
					{
						...
						'<a id="modify" href="#" class="glyphicon glyphicon-repeat" aria-hidden="true"> </a>&nbsp;&nbsp;&nbsp;&nbsp;' +
						'<a id="deleteOne" href="#" class="glyphicon glyphicon-trash" aria-hidden="true"> </a>',
						"sWidth":'10%' 
					}],
		"columnDefs" : [{
			"targets" : [0], // 指定的列
	...
	$('#topPlugin').append(topPlugin);//快捷操作的HTML DOM
       ...
       $("#addUser").on("click",function(){
        ...
        $("#batchDel").on("click",function(){
        		deleteObj($("input[name=userIds]"),'/users/delete',userTable)});//给下方按钮绑定事件
	 ...
	 */
	 $('#example tbody').on('click', '#modify', function() {
		...
		$("#username").val($(this).closest('tr').children("td").eq(1).text());
		$("#nickname").val($(this).closest('tr').children("td").eq(2).text());
		$("#student_id").val($(this).closest('tr').children("td").eq(3).text());
		$("#name").val($(this).closest('tr').children("td").eq(4).text());
		$("#description").val($(this).closest('tr').children("td").eq(5).text());
		$("#credits").val($(this).closest('tr').children("td").eq(6).text());
		$("#email").val($(this).closest('tr').children("td").eq(7).text());
		$("#admin").attr("checked",$(this).closest('tr').children("td").eq(8).text());
		$("#activation").attr("checked",$(this).closest('tr').children("td").eq(10).text());
    ...
    $("#changeUserInfo").on("click",function() {
            $.ajax({
                type: "post",
                url: "/users/update",
                data:{user_id:$("#user_id").val(),username:$("#username").val(),nickname:$("#nickname").val(),
